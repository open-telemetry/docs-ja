...
* [レガシーとモダンなログソース](#レガシーとモダンなログソース)
  ...
  * [レガシーなファーストパーティアプリケーションのログ](#レガシーなファーストパーティアプリケーションのログ)
...
インフラストラクチャ・ログは、Kubernetesのイベントなど、さまざまなインフラストラクチャコンポーネントによって生成されるログです(なぜイベントがログの文脈で議論されるのか不思議に思われる方は、Event and Logs(リンクTBD)をご覧ください)。システムログと同様に、インフラログにはリクエスト・コンテキストがなく、リソース・コンテキスト(ノード、ポッド、コンテナなどに関する情報)によって充実させることができます。
...
OpenTelemetryでは、FluentBitや同様のエージェントを使用してアプリケーションログからログを収集し、OpenTelemetry Collectorに送信(https://github.com/open-telemetry/opentelemetry-collector/tree/master/receiver/fluentforwardreceiver)し、そこでさらに処理して情報を充実することを推奨しています。
...
最初のアプローチでは、ログがファイルや標準出力に書き込まれていると仮定して、ファイルのログを読み込んで更新を監視し、ログローテーションが使用されている場合には正しく動作し、オプションでログを解析してより構造化されたフォーマットに変換する機能が必要となります。Parings(XXX:なんだろうこれは)では、さまざまな種類のパーサーをサポートし、カスタムフォーマットを解析するように設定したり、カスタムパーサーを追加したりすることができます。パーサーがサポートする必要のある一般的なフォーマットの例は以下のとおりです。CSV、Common Log Format、Labeled Tab-separated Values(LTSV)、Key/Value Pairフォーマット、JSONなどです。このアプローチをサポートするために、OpenTelemetryはFluentBitまたは同様のエージェントを使用してログを読み取り、[その後ログを](https://github.com/open-telemetry/opentelemetry-collector/tree/master/receiver/fluentforwardreceiver)OpenTelemetry Collectorに送信することを推奨します。

<!--
![Application to File Logs](img/app-to-file-logs.png)
-->

![アプリケーションからファイルログ](img/app-to-file-logs.png)
...
先に述べたように、OpenTelemetryはパーサーやログファイルの読み込み機能を実装するつもりはないので、この方法を選択した場合、FluentBitのような外部のログ収集エージェントを使用してログを収集する必要があります。
...
- [ログデータモデル](data-model.md)に基づくログデータタイプとログパイプラインのサポート。
...
- テキストファイルからのログの読み込み、ファイルの更新検知処理、一般的なログローテーション方式の理解、ログファイル作成のためのディレクトリの監視、ファイルの位置のチェックポイント、チェックポイントからの読み込みの再開などの機能。この機能は、外部で動作するFluentBitプロセス(または他の類似した外部ログ収集エージェント)を使用して実装されています。
...
- 一般的なテキスト形式でログを解析し、必要に応じてエンドユーザーが解析形式をカスタマイズしたり、カスタムパーサーを追加できること。これにはFluentBitまたは同様のエージェントが使用されます。
...
- Syslogなどのログ用の一般的なネットワークプロトコルでログを受信し、本仕様で定義されているセマンティック規約に基づいて解釈する機能。この機能には、FluentBitなどのエージェントが使用されます。将来的には、この機能の一部がCollectorに直接移行される可能性があります。
